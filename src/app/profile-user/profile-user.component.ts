...
export class ProfileUserComponent {
  ...
  deleteSkill(index: number, skillId: any) {
    if (confirm("Are you sure?")) {
      if (this.appUser.skills) this.appUser.skills.splice(index, 1);
      this.appUserService.deleteSkill(this.appUser.id, skillId).subscribe(data => {
      });
    }
  };
